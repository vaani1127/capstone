...
A defining differentiator of HealthSaathi is its **blockchain integrity layer** for medical records. Every consultation note and prescription entry is cryptographically linked in a SHA-256 hash chain, ensuring that any unauthorized modification is instantly detectable. Complementing this is a **machine-learning behavioural anomaly detection layer** that scores every audit event against a role-specific IsolationForest model trained on 30 days of activity, providing real-time insider-threat and abnormal-access detection with SHAP-based natural-language explanations pushed live to admins via WebSocket. Combined with real-time WebSocket notifications, a comprehensive audit trail, and a deployment-ready Docker and AWS Terraform infrastructure, HealthSaathi delivers enterprise-grade functionality in a package suitable for mid-sized healthcare institutions. With over 556 automated tests and greater than 85% code coverage, the system is built with production reliability in mind.
...
4. **Implement a machine-learning behavioural anomaly detection layer** that scores every audit event with a role-specific IsolationForest model, attributes suspicious features via SHAP, classifies alerts into LOW/MEDIUM/HIGH severity tiers, and delivers real-time MEDIUM/HIGH alerts to admins over a dedicated WebSocket channel with a Flutter admin dashboard for triage and acknowledgement.

5. **Validate the system through comprehensive automated testing and deployment readiness**, achieving over 85% test coverage across backend (target: 391+ tests) and mobile (target: 165+ tests), and produce a containerized, production-ready deployment using Docker Compose and AWS Terraform infrastructure scripts.
...
- **WebSocket server**: JWT-authenticated connections, broadcast of queue, appointment, and anomaly alert events
- **Audit module**: Action logging, paginated log retrieval, CSV/JSON export, tampering alerts
- **Anomaly Detection Service**: IsolationForest per-role scoring, SHAP explainability, severity classification, NL explanation generation, real-time admin WebSocket push
...
### 5.5.1 Behavioural Anomaly Detection

**Motivation.** Blockchain tamper detection addresses *external* data integrity — it detects whether a record was altered after creation. It does not, however, detect an authorized user behaving abnormally: a doctor bulk-accessing hundreds of patient records at 3 a.m., a nurse rapidly editing the same prescription multiple times, or an admin querying records across departments they have no clinical connection to. These insider-threat and abnormal-access patterns are the focus of the anomaly detection layer.

**ML Approach.** An `IsolationForest` model (scikit-learn 1.4.2) is trained independently for each of the four user roles (Doctor, Nurse, Patient, Admin) using the last 30 days of audit chain activity. Isolation Forest is chosen for its suitability to unlabelled anomaly detection and its low inference latency, both critical requirements for a background task that must not slow down primary API responses. The model is lazily trained on first use per role and cached in a module-level dict; no pre-training or model artefact management is required.

Eight features are extracted from each user's recent audit window:

| Feature | Description |
|---------|-------------|
| `actions_per_hour` | Rate of audit-logged actions in the current session window |
| `unique_patients_accessed` | Distinct patient records touched |
| `off_hours_flag` | 1 if action occurred outside 07:00–21:00 |
| `untreated_patient_ratio` | Fraction of accessed patients with no prior appointment with this user |
| `record_type_entropy` | Shannon entropy over `record_type` values |
| `rapid_edit_flag` | 1 if two or more edits to the same record occurred within 60 seconds |
| `cross_role_action_flag` | 1 if the action type is atypical for the user's role |
| `session_duration_minutes` | Duration of the inferred session |

**Explainability.** SHAP `TreeExplainer` (shap 0.45.1) is applied to every scored event to produce a top-3 feature attribution vector. A template-based natural-language generator converts these attributions into a human-readable explanation sentence (e.g., *"User accessed 14 distinct patients outside regular hours with rapid successive edits."*) stored in the `explanation` column of `anomaly_alerts`.

**Severity Tiers.** Scores (0–1) are mapped to three severity levels:
...
Alerts are only persisted when the score exceeds 0.50. The `analyze_and_alert()` orchestrator is fully wrapped in try/except so a model failure never affects the primary API response.

**Real-Time Admin Alerts.** The existing `ConnectionManager` WebSocket class is extended with `connect_admin`, `disconnect_admin`, and `broadcast_anomaly_alert` methods. When a MEDIUM or HIGH alert is created, it is immediately pushed as a JSON message to all connected admin WebSocket clients at `WS /api/v1/anomaly/ws/admin`. LOW-severity alerts are available only via the REST `GET /anomaly/alerts` endpoint.

**Flutter Admin UI.** `anomaly_alerts_screen.dart` fetches the alert list and stats in parallel on screen load, then subscribes to the WebSocket for live updates. New alerts are prepended to the list without a full reload. A colour-coded stats bar (Total / Unread / HIGH) sits above a scrollable list of alert cards, each showing the severity badge, SHAP explanation, score as a percentage, timestamp, and an **Acknowledge** button that triggers an optimistic UI update before the `POST /anomaly/alerts/{id}/acknowledge` response returns.

### 5.6 Phase 5 — Testing and Deployment

**Backend testing (391 tests)** using Pytest covers:
...
- Anomaly detection — feature extraction, scoring, severity, NL explanation, REST API (18 tests)
...
4. **Automated Test Suite** — 556 automated tests (391 backend + 165 mobile) with >85% code coverage, runnable with a single command.
...
| **Machine Learning** | IsolationForest anomaly detection, SHAP value computation (TreeExplainer), feature engineering from time-series audit logs, severity classification thresholds |
