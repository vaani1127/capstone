...
1. **Backend API** (FastAPI + Python)
   ...
   - ML anomaly detection service (IsolationForest + SHAP) for behavioural threat detection
...
- **IsolationForest** model trained per user role (Doctor / Nurse / Patient / Admin) on 30 days of audit activity — detects insider threats and abnormal access patterns
- **8 behavioural features** including off-hours access, rapid edits, patient access breadth, and record-type entropy
- **SHAP TreeExplainer** attributes the top-3 contributing features for every alert, surfaced as a natural-language explanation
- **Three severity tiers** — LOW (score < 0.60), MEDIUM (< 0.75), HIGH (≥ 0.75)
- **Real-time WebSocket push** of MEDIUM/HIGH alerts to connected admins via `WS /anomaly/ws/admin`
- **Flutter admin dashboard** with stats bar, colour-coded alert cards, and per-card acknowledgement
...
- **Total Tests**: 556 (391 backend + 165 mobile)
- **Test Coverage**: >85%
- **API Endpoints**: 35+
- **Database Tables**: 7
- **Mobile Screens**: 13
